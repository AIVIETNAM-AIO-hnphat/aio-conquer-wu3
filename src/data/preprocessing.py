...
def b2_handle_outliers(
    ...
    """
    Xử lý outlier theo 2 lượt tuần tự.

    Lượt 1 - clip theo ngưỡng lâm sàng: loại giá trị vô lý do lỗi nhập liệu. Clip không thay đổi mask

    Lượt 2 - IQR×3: phát hiện outlier thống kê còn sót trong vùng hợp lệ.
                    Giá trị bị phát hiện được thay bằng NaN và mask tương ứng về 0.

    Args:
        df      : DataFrame sau khi đã tạo mask (B1)
        mask_df : mask DataFrame từ B1
        cfg     : cấu hình pipeline

    Returns:
        (df_clean, mask_df_updated)
    """
...
def run_pipeline(cfg: PipelineConfig) -> Dict[str, Dict[str, np.ndarray]]:
    """
    Thực thi toàn bộ pipeline B1–B8 tuần tự

    Args:
        cfg : cấu hình pipeline, bao gồm data_path và output_dir

    Returns:
        Dict splits {"train", "val", "test"}, mỗi tập chứa X, M, y, stay_ids, X_norm sau khi xử lý đầy đủ
    """
